...
material's own A. The limits make the interpolation explicit: `a→0 ⇒ B_a→A = 1/ε∞` (a
global hybrid with a single fraction 1/ε∞) and `a→∞ ⇒ B_a→1` (DD-RSH-CAM). For finite `a`,
`A ≤ B_a ≤ 1`, and larger `a` means *more* short-range Fock. The dielectric input is
