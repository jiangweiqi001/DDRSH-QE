#!/bin/bash
# Scan turboEELS over q to get eps^-1(q) at omega=0 (PBE, RPA+local fields) for Si.
source /home/footman/miniconda3/etc/profile.d/conda.sh
conda activate qedev
export OMP_NUM_THREADS=4
cd /home/footman/DDRSH/runs/Si/p2/eels
...
EOF
  ...
  line=$(grep -v '^#' si.plot_eps.dat | head -1)
  ...
  cp si.plot_eps.dat plot_eps.q${q}.dat
done
echo "=== eps_q.dat ==="
cat "$OUT"
echo DONE
